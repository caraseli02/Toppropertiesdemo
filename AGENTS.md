...
- Playwright E2E tests will be restored when the new generated app lands; the reset baseline runs L1+L2 only.
...
Every PR must pass `npm run verify` before opening. For UI work, also check the primary flow in a browser at desktop and 375px. Re-add Playwright E2E once the new app surface is integrated.
...
| **L3 E2E**       | Playwright (TBD) | Full browser flow through real dev server             | Restore with the new generated app |

- **Unit/component tests** live in `src/**/*.test.ts(x)`, run via Vitest with jsdom.
- **E2E tests** were removed during the 2026-06-30 harness-only reset; restore `e2e/*.spec.ts` when the new app lands.
- CI currently runs L1+L2 as part of `npm run verify`.
...
| `docs/design/pencil-design-system-rules.md` | Pencil MCP workflow, `design.pen` component catalog, token mapping, and `nIs4H` reference target.     |
