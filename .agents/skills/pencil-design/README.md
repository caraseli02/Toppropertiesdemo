...
- **Code generation** — Producing React + Tailwind v4 code aligned with existing project primitives
...
### 6. Always Load Local Product and Design Context
**Never design without aesthetic direction.** Read local product context, tokens, and components first so designs avoid generic AI aesthetics.
...
```
0. Read local product/design context -> Get aesthetic direction
...
1. Read `CONTEXT.md`, `src/index.css`, and relevant components
...
5. Map Pencil components to existing React primitives first
6. Generate code using semantic Tailwind classes
7. Apply local product/design context for typography, color, motion
...
- **Components**: Existing project React primitives first; shadcn/ui only if explicitly added later
- **Language**: TypeScript
- **Icons**: Lucide React
- **Utilities**: `cn()` from `@/utils/cn`
- **React Version**: 19
...
└── references/
    ...
    ├── tailwind-shadcn-mapping.md     # Legacy quick-reference mapping tables
