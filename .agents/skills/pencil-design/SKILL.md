...
- Syncing design tokens between Pencil and code (Tailwind v4 `@theme` tokens)
...
### Rule 6: Always Load Local Product and Design Context

**NEVER design in Pencil or generate code from Pencil without first reading the repo's product/design context.**

This repo does not currently include a `frontend-design` skill. You MUST instead:
1. Read `CONTEXT.md` for product scope, persona, and glossary
2. Read `src/index.css` for Tailwind v4 `@theme` tokens
3. Inspect relevant `src/components/*` before translating a Pencil design to code
4. Call `pencil_get_guidelines` for the relevant design/code topic when Pencil is available
5. Never produce generic AI aesthetics (overused fonts, cliched color schemes, predictable layouts)
...
```
0. Read local context             -> CONTEXT.md, src/index.css, relevant components
...
See [references/tailwind-shadcn-mapping.md](references/tailwind-shadcn-mapping.md) for the legacy Pencil-to-Tailwind mapping table.
...
Summary:
1. Read local product/design context for aesthetic direction
...
5. Map reusable Pencil components to existing React primitives first
6. Generate code using semantic Tailwind classes (`bg-primary`, `rounded-md`), never arbitrary values
7. Apply local product/design context: distinctive typography, intentional color, motion, spatial composition
8. Use typed local components, `cn()` for class merging, and Lucide for icons
...
| Skipping local product/design context | Read `CONTEXT.md`, `src/index.css`, and relevant components first |
| Generic AI aesthetics (Inter font, purple gradients) | Follow local product/design context and Pencil guidelines |
